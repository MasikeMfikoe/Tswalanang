...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    console.error("Error in client portal order details GET:", error)
...
export async function PUT(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { data, error } = await supabase.from("orders").update(allowedUpdates).eq("id", id).select().single()
    ...
    console.error("Error in client portal order details PUT:", error)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}
