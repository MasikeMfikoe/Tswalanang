...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json({ data, success: true, message: "Estimate updated successfully" })
  } catch (error) {
...
export async function DELETE(
  ...
  try {
    ...
    if (id.startsWith("est-") && /^est-\d+$/.test(id)) {
      return NextResponse.json({ error: "Cannot delete sample data.", success: false }, { status: 400 })
    }
