...
export default function CourierOrderDetailsClient({ id }: { id: string }) {
  ...
  useEffect(() => {
    const fetchCourierOrderDetails = async () => {
      try {
        ...
        console.log("[v0] Fetching courier order details via API for ID:", id)
        const response = await fetch(`/api/courier-orders/${id}`)
        ...
        console.error("[v0] Error fetching courier order:", err)
    ...
    if (id) fetchCourierOrderDetails()
  }, [id])
