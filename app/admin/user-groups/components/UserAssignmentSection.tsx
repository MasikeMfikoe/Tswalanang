...
export default function UserAssignmentSection({ groupId, isDefaultGroup }: UserAssignmentSectionProps) {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      const assignedIds: string[] = (links ?? []).map((r: any) => r.user_id).filter(Boolean)

      // 2) Load all user profiles
      const { data: profiles, error: profErr } = await supabase.from("user_profiles").select(PROFILE_SELECT)
